...
import com.example.wedbansach_bakend1.dao.NguoiDungRepository; // <<< THÊM IMPORT
import com.example.wedbansach_bakend1.entity.NguoiDung;
import com.example.wedbansach_bakend1.entity.Quyen;
import io.jsonwebtoken.Claims;
import io.jsonwebtoken.Jwts;
...
public class JwtService {
    ...
    public String generateToken(String tenDangNhap){
        ...
        // Lấy thông tin người dùng và các quyền thật từ DB
        NguoiDung nguoiDung = nguoiDungRepository.findByTenDangNhap(tenDangNhap);
        if (nguoiDung != null && nguoiDung.getDanhSachQuyen() != null) {
            // Chuyển danh sách Quyen (Entity) thành danh sách String (tên quyền)
            List<String> roles = nguoiDung.getDanhSachQuyen()
                    .stream()
                    .map(Quyen::getTenQuyen) // Lấy tên của mỗi quyền
                    .collect(Collectors.toList());

            // Thêm danh sách quyền và các boolean kiểm tra nhanh vào token
            claims.put("roles", roles);
            claims.put("isAdmin", roles.contains("admin")); // Sẽ là true nếu list roles chứa "admin"
            claims.put("isUser", roles.contains("user"));   // Sẽ là true nếu list roles chứa "user"
        }

        // Tạo token với các quyền thật
        return createToken(claims, tenDangNhap);
    }
