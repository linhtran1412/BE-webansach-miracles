...
public interface DonHangRepository extends JpaRepository<DonHang, Integer> {
    ...
    @Query("SELECT COALESCE(SUM(d.tongTien), 0.0) FROM DonHang d WHERE d.ngayTao BETWEEN :from AND :to")
    Double sumRevenue(@Param("from") LocalDate from, @Param("to") LocalDate to);

    @Query("SELECT COUNT(d) FROM DonHang d WHERE d.ngayTao BETWEEN :from AND :to")
    Long countOrders(@Param("from") LocalDate from, @Param("to") LocalDate to);

    @Query("SELECT d.trangThai, COUNT(d) FROM DonHang d WHERE d.ngayTao BETWEEN :from AND :to GROUP BY d.trangThai")
    List<Object[]> countByStatus(@Param("from") LocalDate from, @Param("to") LocalDate to);

    @Query("SELECT d.ngayTao, COALESCE(SUM(d.tongTien), 0.0) FROM DonHang d WHERE d.ngayTao BETWEEN :from AND :to GROUP BY d.ngayTao ORDER BY d.ngayTao")
    List<Object[]> revenueByDay(@Param("from") LocalDate from, @Param("to") LocalDate to);

    // List đơn của 1 user, sort mới → cũ
    List<DonHang> findByNguoiDung_MaNguoiDungOrderByNgayTaoDesc(int maNguoiDung);

    // (tuỳ chọn) theo username
    List<DonHang> findByNguoiDung_TenDangNhapOrderByNgayTaoDesc(String tenDangNhap);

    // === BỔ SUNG CHO TRANG CHI TIẾT USER ===
    Optional<DonHang> findByMaDonHangAndNguoiDung_MaNguoiDung(int maDonHang, int maNguoiDung);

    @Query("""
           SELECT d FROM DonHang d
           LEFT JOIN FETCH d.danhSachChiTietDonHang c
           LEFT JOIN FETCH c.sach s
           WHERE d.maDonHang = :id
           """)
    Optional<DonHang> fetchWithDetails(@Param("id") int id);
}
